...
public class FeedParser
{
    ...
    /**
     * Traverses the JSOUP XML Document(Rss or Atom feed) selects each
     * individual feed entry element and fills a ArrayList with the text values
     * of title,link and description elements. The ArrayList is filled using a
     * simple FeedEntry object(plain container for the extracted String values)
     * 
     * @param url
     *            web Feed Url Address
     * @return ArrayList(FeedEntry) - Each FeedEntry object has getters for
	...
	if (xmlDoc.select("rss") != null)
	{
		...
		rssEntry.setDescription(e.getElementsByTag("description")
			.text());
	...
	// process ATOM feeds
	if ((root = xmlDoc.select("feed")) != null
		&& root.attr("xmlns").contains("http://www.w3.org/2005/Atom"))
	{
