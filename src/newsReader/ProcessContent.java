...
public class ProcessContent extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException 
	{
		...
		ArrayList<String> feedLinks = (ArrayList<String>) session.getAttribute("FeedLinks");
		UrlInfo info = (UrlInfo)session.getAttribute("UrlInfo");
	/*
		for(String link : feedLinks)
		{
		 results = parser.parseFeedFromUrl(link);
			
	     for(FeedEntry e : results)
	      {
	    	 if(matchingKeywors(info.getTitle(), e.getTitle()) > 0)
	    	{
	    	 writer.println(e.getTitle());
	    	 writer.println("<br>");
	    	 writer.println(e.getDescription());
	    	 writer.println("<br>");
	    	 writer.println(e.getLink());
	    	 writer.println("<br>");
	    	}
	...
	/**
	 * Sorts the returned WordOccurence objects by the matching words count(Descending) from the Description meta tags 
	 * <p>
	 * Compares the search url Description meta tag with the corresponding Description tag 
	 * from  each individual Feed item.
	 * 
	 * @param entries ArrayList of all the feed entries from one Feed
	 * @param info UrlInfo object used to get the description and title from the entered search url
	 * @return List of WordOccurence objects containing the matching words 
	 * and the corresponding words count
	 */
	private ArrayList<WordOccurence> sortByContent(ArrayList<FeedEntry> entries,UrlInfo info )
	{
		...
		{
		  int matches = matchingKeywors(info.getDescription(), e.getDescription());
	 ...
	 * 
	 * @param urlData String from the search url (Title or Description)
	 * @param feedData String from the Feed entry(Title or Description)
